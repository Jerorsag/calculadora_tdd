...
FASE 3 - REFACTOR
============================================================
Se mantienen los tests de las fases anteriores SIN cambios.
Se agregan los nuevos tests de TestValidaciones, escritos
durante el refactor para cubrir la nueva logica de validacion.
...
# ----------------------------------------------------------
# Tests heredados de Fase 1 y Fase 2 (sin modificaciones)
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# NUEVOS tests agregados en el REFACTOR
# Cubren la validacion de tipos implementada en _validar_numeros()
# ----------------------------------------------------------


class TestValidaciones(unittest.TestCase):
    """
    Tests de validacion de tipo de dato.
    Escritos durante el REFACTOR al extraer _validar_numeros().
    """

    def setUp(self):
        self.calc = Calculadora()

    def test_sumar_con_string_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.sumar("cinco", 3)

    def test_restar_con_none_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.restar(None, 5)

    def test_multiplicar_con_lista_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.multiplicar([1, 2], 3)

    def test_dividir_con_string_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.dividir(10, "dos")

    def test_raiz_cuadrada_con_string_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.raiz_cuadrada("nueve")

    def test_potencia_con_string_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.potencia("dos", 3)

    def test_modulo_con_none_lanza_type_error(self):
        with self.assertRaises(TypeError):
            self.calc.modulo(10, None)
